...
def parse_points(point_str):
    ...
    try:
        ...
        return sorted(points)  # Importante: orden ascendente en x
    except:
...
def natural_cubic_spline(points, x_query):
    ...
    # Paso 3: Construcción del sistema lineal para los C_i (C_0 = C_n = 0)
    A = np.zeros((n, n))
    b = np.zeros(n)
    ...
    # Paso 4: Coeficientes a, b, d
    a = y[:-1]
    b_coef = m - h * (2 * C[:-1] + C[1:]) / 3
    d = (C[1:] - C[:-1]) / (3 * h)

    # Construcción de los polinomios por intervalos
    polinomios = []
    for i in range(n - 1):
        polinomios.append({
