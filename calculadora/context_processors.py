...
def language_context(request):
    ...
    translations = {
        'es': {
            ...
            # Cubic Spline Documentation Keys
            'cubic_spline_interpolation': 'Interpolación de Trazadores Cúbicos',
            'cubic_spline_description': 'Construir una función que interpole un conjunto de puntos dados, asegurando que sea continua en la función, primera y segunda derivada en cada punto.',
            'algorithm_steps': 'Pasos del Algoritmo',
            'input_data': 'Datos de entrada',
            'ordered_points_set': 'Conjunto de n + 1 puntos ordenados',
            'calculate_subinterval_lengths': 'Calcular longitudes de subintervalos',
            'equations_system_for_mi': 'Sistema de ecuaciones para M_i',
            'natural_condition': 'Condición natural: M_0 = M_n = 0',
            'solve_linear_system': 'Resolver el sistema lineal',
            'to_obtain_m_values': 'para obtener M_0, M_1, ..., M_n',
            'cubic_spline_formula': 'Fórmula del trazador cúbico S_i(x)',
            'theoretical_example': 'Ejemplo Teórico',
            'step': 'Paso',
            'initial_data': 'Datos iniciales',
            'subintervals': 'Subintervalos: h_0 = h_1 = h_2 = 1',
            'construct_system_second_derivatives': 'Construir el sistema para las segundas derivadas M_i',
            'general_formula_tridiagonal': 'La fórmula general para construir el sistema tridiagonal es',
            ...
            'max_10_variables': 'máx. 10 variables',
            'example_objective_placeholder': 'Ej: 2,3,1,0',
            'variable_limit_reached': 'Ya consumió su límite de 10 variables.',
            'constraint_limit_reached': 'Ya consumió su límite de 10 restricciones.',
            'add_constraint': 'Agregar restricción',
            'solve': 'Resolver',
            'constraint_label': 'Restricción',
            'constraint_placeholder': 'Ej: 1,2',
            'constraint_value_placeholder': 'Ej: 10',
            'confirm_delete_constraint': '¿Desea eliminar esta restricción?',
            'objective_function_required': 'La función objetivo no puede estar vacía.',
            'max_10_variables_allowed': 'Máximo 10 variables permitidas en la función objetivo.',
            'coefficients_must_be_numeric': 'Todos los coeficientes en la función objetivo deben ser numéricos.',
            'constraint_values_mismatch': 'La restricción',
            'must_have_values': 'debe tener',
            'including_right_side': 'valores (incluyendo el lado derecho).',
            'constraint_values_numeric': 'Todos los valores en la restricción',
            'must_be_numeric': 'deben ser numéricos.',
            'big_m_method_desc': 'Método para resolver problemas de programación lineal con restricciones de igualdad',
            'graphic_method_desc': 'Método visual para resolver problemas de programación lineal con dos variables',
            
            # Big M History - Spanish
            'big_m_history': 'Historial Gran M',
            'big_m_history_title': 'Historial de Métodos - Gran M',
            'type': 'Tipo',
            'objective': 'Objetivo',
            'view_results': 'Ver resultados',
            'edit': 'Editar',
            'no_calculations_registered': 'No hay cálculos registrados aún.',
            ...
            # Método Gráfico - Traducciones específicas del template
            'back_to_home': 'Volver al Inicio',
            'viewing_history_calculation': 'Estás viendo una repetición de un cálculo del historial.',
            'back_to_history': 'Volver al Historial',
            'history_mode': 'Modo Repetición',
            'view_history': 'Ver Historial',
            'new_exercise': 'Nuevo ejercicio',
            'errors': 'Errores',
            'input_data': 'Datos de Entrada',
            'objective_function': 'Función Objetivo (Z)',
            'example_objective': 'Ejemplo: 3*x + 2*y',
            'optimization_type': 'Tipo de Optimización',
            'maximize': 'Maximizar',
            'minimize': 'Minimizar',
            'constraints': 'Restricciones (una por línea)',
            'example_constraint': 'Ejemplo: 2*x + y <= 100',
            'calculate_solution': 'Calcular Solución',
            'enter_constraints_tip': 'Ingresa tus restricciones y obtén la solución óptima',
            'results': 'Resultados',
            'historial': 'Historial',
            'history_mode': 'Modo Historial',
            
            # Método Gráfico - Historial
            'graphic_method_history': 'Historial - Método Gráfico',
            'history_graphic_method': 'Historial del Método Gráfico',
            'no_history_records': 'No hay registros en el historial.',
            'creation_date': 'Fecha',
            'objective_function': 'Función Objetivo',
            ...
            'limits': 'Límites',
            'solution': 'Solución',
            'optimal_point': 'Punto Óptimo',
            'actions': 'Acciones',
            'download_pdf': 'Descargar PDF',
            'repeat': 'Repetir',
            'back_to_method': 'Volver al Método',
            'detailed_procedure': 'Procedimiento Detallado',
            'exercise_number': 'Ejercicio #',
            'step': 'Paso',
            'see_system_development': 'Ver desarrollo del sistema',
            'stored_procedure': 'Procedimiento',
            'no_detailed_procedure': 'No hay procedimiento detallado guardado para este ejercicio.',
            'graphic_representation': 'Representación Gráfica',
            'want_more_details': '¿Quieres ver más detalles y gráficas en tus cálculos?',
            'upgrade_to_premium': 'Actualiza a Premium',
            
            # Tienda - Premium Store
            'forja_solver_premium': 'Forja Solver Premium',
            'unlock_full_potential': 'Desbloquea todo el potencial de nuestros modelos matemáticos',
            'complete_math_processes': 'Procesos matemáticos completos',
            'step_by_step_visualization': 'Visualización paso a paso',
            'interactive_graphs': 'Gráficas interactivas',
            'detailed_explanations': 'Explicaciones detalladas',
            'export_to_pdf': 'Exportar a PDF',
            'priority_support': 'Soporte prioritario',
            'already_premium': '¡Ya disfrutas de Forja Solver Premium!',
            'back_to_home': 'Volver al inicio',
            'monthly': '/mes',
            ...
            'maximization': 'Maximización',
            'minimization': 'Minimización',
            'feasible_points_evaluated': 'Puntos Factibles Evaluados',
            'point': 'Punto',
            'x_coordinate': 'Coordenada X',
            'y_coordinate': 'Coordenada Y',
            'detailed_procedure': 'Procedimiento Detallado',
            'stored_procedure': 'Procedimiento Almacenado',
            'step_1_problem_definition': 'Paso 1: Definición del Problema',
            'problem_established': 'Se establece el problema de programación lineal con:',
            'objective_function_label': 'Función objetivo',
            'optimization_type_label': 'Tipo de optimización',
            'restrictions_label': 'Restricciones',
            'step_2_intersection_points': 'Paso 2: Identificar Puntos de Intersección',
            'intersection_calculation': 'Se calculan los puntos de intersección entre las restricciones:',
            'intersection_between': 'Intersección entre:',
            'point_obtained': 'Punto obtenido',
            'step_3_verify_feasibility': 'Paso 3: Verificar Factibilidad',
            'verify_feasibility_desc': 'Se verifican qué puntos satisfacen todas las restricciones:',
            'step_4_evaluate_objective': 'Paso 4: Evaluar Función Objetivo',
            'evaluate_objective_desc': 'Se evalúa Z en cada punto factible:',
            'step_5_determine_optimal': 'Paso 5: Determinar Solución Óptima',
            'determine_optimal_desc': 'Se identifica la solución óptima:',
            'maximum_value': 'El valor máximo',
            'minimum_value': 'El valor mínimo',
            'of_z_is': 'de Z es',
            'reached_at_point': 'Se alcanza en el punto',
            'graphic_representation': 'Representación Gráfica',
            'graph_info': 'La gráfica muestra la región factible (área verde), las restricciones (líneas azules) y el punto óptimo (estrella dorada).',
            'premium_detailed_procedure': 'Procedimiento Detallado Premium',
            'premium_unlock_message': 'Desbloquea el procedimiento paso a paso y la representación gráfica interactiva con una suscripción premium.',
            'upgrade_to_premium': 'Actualizar a Premium',
            'premium_interactive_graph': 'Gráfico Interactivo Premium',
            'premium_graph_message': 'Visualiza la región factible, restricciones y punto óptimo con nuestra herramienta gráfica interactiva.',
            'client_validation_message': 'Por favor ingresa al menos una restricción',
        ...
        'en': {
            ...
            'cubic_spline_description': 'Construct a function that interpolates a given set of points, ensuring it is continuous in the function, first and second derivative at each point.',
            'algorithm_steps': 'Algorithm Steps',
            'input_data': 'Input data',
            'ordered_points_set': 'Set of n + 1 ordered points',
            'calculate_subinterval_lengths': 'Calculate subinterval lengths',
            'equations_system_for_mi': 'System of equations for M_i',
            'natural_condition': 'Natural condition: M_0 = M_n = 0',
            'solve_linear_system': 'Solve the linear system',
            'to_obtain_m_values': 'to obtain M_0, M_1, ..., M_n',
            'cubic_spline_formula': 'Cubic spline formula S_i(x)',
            'theoretical_example': 'Theoretical Example',
            'step': 'Step',
            'initial_data': 'Initial data',
            'subintervals': 'Subintervals: h_0 = h_1 = h_2 = 1',
            'construct_system_second_derivatives': 'Construct the system for second derivatives M_i',
            'general_formula_tridiagonal': 'The general formula to construct the tridiagonal system is',
            ...
            'max_10_variables': 'max. 10 variables',
            'example_objective_placeholder': 'Ex: 2,3,1,0',
            'variable_limit_reached': 'You have reached your limit of 10 variables.',
            'constraint_limit_reached': 'You have reached your limit of 10 constraints.',
            'add_constraint': 'Add constraint',
            'solve': 'Solve',
            'constraint_label': 'Constraint',
            'constraint_placeholder': 'Ex: 1,2',
            'constraint_value_placeholder': 'Ex: 10',
            'confirm_delete_constraint': 'Do you want to delete this constraint?',
            'objective_function_required': 'The objective function cannot be empty.',
            'max_10_variables_allowed': 'Maximum 10 variables allowed in the objective function.',
            'coefficients_must_be_numeric': 'All coefficients in the objective function must be numeric.',
            'constraint_values_mismatch': 'Constraint',
            'must_have_values': 'must have',
            'including_right_side': 'values (including the right-hand side).',
            'constraint_values_numeric': 'All values in constraint',
            'must_be_numeric': 'must be numeric.',
            'big_m_method_desc': 'Method to solve linear programming problems with equality constraints',
            'graphic_method_desc': 'Visual method to solve linear programming problems with two variables',
            
            # Big M History - English
            'big_m_history': 'Big M History',
            'big_m_history_title': 'Method History - Big M',
            'type': 'Type',
            'objective': 'Objective',
            'view_results': 'View results',
            'edit': 'Edit',
            'no_calculations_registered': 'No calculations registered yet.',
            ...
            # Graphic Method - Template specific translations
            'back_to_home': 'Back to Home',
            'viewing_history_calculation': 'You are viewing a repetition of a calculation from history.',
            'back_to_history': 'Back to History',
            'history_mode': 'Repetition Mode',
            'view_history': 'View History',
            'new_exercise': 'New exercise',
            'errors': 'Errors',
            'input_data': 'Input Data',
            'objective_function': 'Objective Function (Z)',
            'example_objective': 'Example: 3*x + 2*y',
            'optimization_type': 'Optimization Type',
            'maximize': 'Maximize',
            'minimize': 'Minimize',
            'constraints': 'Constraints (one per line)',
            'example_constraint': 'Example: 2*x + y <= 100',
            'calculate_solution': 'Calculate Solution',
            'enter_constraints_tip': 'Enter your constraints and get the optimal solution',
            'results': 'Results',
            'historial': 'History',
            'history_mode': 'History Mode',
            
            # Graphic Method - History
            'graphic_method_history': 'History - Graphic Method',
            'history_graphic_method': 'Graphic Method History',
            'no_history_records': 'No records in history.',
            'creation_date': 'Date',
            'objective_function': 'Objective Function',
            ...
            'step': 'Step',
            'see_system_development': 'See system development',
            'stored_procedure': 'Procedure',
            'no_detailed_procedure': 'No detailed procedure saved for this exercise.',
            'graphic_representation': 'Graphic Representation',
            'want_more_details': 'Want to see more details and graphs in your calculations?',
            'upgrade_to_premium': 'Upgrade to Premium',
            
            # Premium Store
            'forja_solver_premium': 'Forja Solver Premium',
            'unlock_full_potential': 'Unlock the full potential of our mathematical models',
            'complete_math_processes': 'Complete mathematical processes',
            'step_by_step_visualization': 'Step-by-step visualization',
            'interactive_graphs': 'Interactive graphs',
            'detailed_explanations': 'Detailed explanations',
            'export_to_pdf': 'Export to PDF',
            'priority_support': 'Priority support',
            'already_premium': 'You already enjoy Forja Solver Premium!',
            'back_to_home': 'Back to home',
            'monthly': '/month',
            ...
            'maximization': 'Maximization',
            'minimization': 'Minimization',
            'feasible_points_evaluated': 'Feasible Points Evaluated',
            'point': 'Point',
            'x_coordinate': 'X Coordinate',
            'y_coordinate': 'Y Coordinate',
            'detailed_procedure': 'Detailed Procedure',
            'stored_procedure': 'Stored Procedure',
            'step_1_problem_definition': 'Step 1: Problem Definition',
            'problem_established': 'The linear programming problem is established with:',
            'objective_function_label': 'Objective function',
            'optimization_type_label': 'Optimization type',
            'restrictions_label': 'Restrictions',
            'step_2_intersection_points': 'Step 2: Identify Intersection Points',
            'intersection_calculation': 'Intersection points between restrictions are calculated:',
            'intersection_between': 'Intersection between:',
            'point_obtained': 'Point obtained',
            'step_3_verify_feasibility': 'Step 3: Verify Feasibility',
            'verify_feasibility_desc': 'We verify which points satisfy all restrictions:',
            'step_4_evaluate_objective': 'Step 4: Evaluate Objective Function',
            'evaluate_objective_desc': 'Z is evaluated at each feasible point:',
            'step_5_determine_optimal': 'Step 5: Determine Optimal Solution',
            'determine_optimal_desc': 'The optimal solution is identified:',
            'maximum_value': 'The maximum value',
            'minimum_value': 'The minimum value',
            'of_z_is': 'of Z is',
            'reached_at_point': 'It is reached at point',
            'graphic_representation': 'Graphic Representation',
            'graph_info': 'The graph shows the feasible region (green area), constraints (blue lines) and optimal point (golden star).',
            'premium_detailed_procedure': 'Premium Detailed Procedure',
            'premium_unlock_message': 'Unlock the step-by-step procedure and interactive graphical representation with a premium subscription.',
            'upgrade_to_premium': 'Upgrade to Premium',
            'premium_interactive_graph': 'Premium Interactive Graph',
            'premium_graph_message': 'Visualize the feasible region, constraints and optimal point with our interactive graphical tool.',
            'client_validation_message': 'Please enter at least one constraint',
